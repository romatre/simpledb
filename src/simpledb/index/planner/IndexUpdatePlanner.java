...
/**
 * A modification of the basic update simpledb.planner.
 * It dispatches each update statement to the corresponding
 * simpledb.index simpledb.planner.
...
public class IndexUpdatePlanner implements UpdatePlanner {
   ...
   public int executeInsert(InsertData data, Transaction tx) {
      ...
      // first, insert the simpledb.record
      UpdateScan s = (UpdateScan) p.open();
      s.insert();
      RID rid = s.getRid();
      
      // then modify each field, inserting an simpledb.index simpledb.record if appropriate
   ...
   public int executeDelete(DeleteData data, Transaction tx) {
      ...
      int count = 0;
      while(s.next()) {
         // first, delete the simpledb.record's RID from every simpledb.index
         RID rid = s.getRid();
         ...
         // then delete the simpledb.record
         s.delete();
         count++;
      }
      s.close();
