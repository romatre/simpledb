 ...
 * from the simpledb.log in reverse order.
 * Unlike the similar class 
 * {@link simpledb.log.LogIterator LogIterator},  
 * this class understands the meaning of the simpledb.log records.
...
class LogRecordIterator implements Iterator<LogRecord> {
   ...
   /**
    * Constructs a simpledb.log simpledb.record from the values in the
    * current basic simpledb.log simpledb.record.
    * The method first reads an integer, which denotes
    * the type of the simpledb.log simpledb.record.  Based on that type,
    * the method calls the appropriate LogRecord constructor
    * to read the remaining values.
    * @return the next simpledb.log simpledb.record, or null if no more records
    */
